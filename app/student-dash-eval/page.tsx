...
export default function StudentDashboard() {
    ...
    useEffect(() => {
        ...
        const savedHidden = localStorage.getItem('hiddenStaff');
        if (savedHidden) setHiddenIds(JSON.parse(savedHidden));

        const handleStorageChange = () => {
            const updatedHidden = localStorage.getItem('hiddenStaff');
            if (updatedHidden) setHiddenIds(JSON.parse(updatedHidden));
        };
    ...
    const filteredInstructors = useMemo(() => {
        ...
        return allInstructors.filter((inst) => {
            ...
            return isMatch && !hiddenIds.includes(inst.id);
        });
    }, [allInstructors, hiddenIds, user]);
    ...
    const renderStars = (rating: number) => {
        return (
            <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className={`text-lg ${star <= rating ? 'text-yellow-400' : 'text-slate-200'}`}>
                        ★
                    </span>
                ))}
            </div>
    ...
    if (loading) return <div className="min-h-screen bg-[#f0f2ff] flex items-center justify-center font-bold text-[#445cf5]">Loading...</div>;
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-slate-400 font-medium">ID: {inst.id_number}</p>
    ...
    className={`text-white text-sm px-5 py-2 rounded-xl font-semibold transition whitespace-nowrap ml-4 ${
        isEvaluated
            ...
            : 'bg-[#445cf5] hover:bg-blue-700'
    }`}
>
    {isEvaluated ? 'Re-evaluate' : 'Start'}
</button>
                                    </div>
                                );
                            })
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100">
                                ...
                                {evaluations.length < filteredInstructors.length && (
                                    <div className="w-full bg-slate-200 rounded-full h-2 mt-2 overflow-hidden">
                                        <div 
                                            className="bg-[#445cf5] h-full transition-all"
